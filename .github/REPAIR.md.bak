...
3. Tests were green **before AND after** the change. A fix that "makes tests
   ...
   fixed anything.
4. At least one check run on the PR's head SHA concluded `success`.
5. Zero check runs concluded `failure`, `cancelled`, or `timed_out`.
6. Check runs concluding `skipped` or `neutral` do **not** count toward
   condition 4, and **NO CHECKS AT ALL is a BLOCK, never a pass** (S5.2).
